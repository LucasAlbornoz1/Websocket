...
class ProductManager {
    ...
    #validateEvent = (title, description, price, thumbnail, code, stock) => {
        if (!title || !description || !price || !thumbnail || !code || !stock){
            this.#error = `Espere ( ${title} : Faltan datos )`
        } else {
            const found = this.#products.find(product => product.code === code)
            if(found) this.#error = `Espere ( ${title} : El producto ya existe )`
    ...
    getProducts = async () => {
        ...
        if (!existe) {
            await fs.promises.writeFile(this.#path, JSON.stringify(this.#products, null, '\t'))
            console.log("No lo teniamos, pero lo creamos recien")
        }
